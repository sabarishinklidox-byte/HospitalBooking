...
export default function BillingPage() {
  const { clinic, plan: currentPlan, reloadAdmin, loading: ctxLoading } =
    useAdminContext();
  ...
  const handleUpgrade = async (targetPlanId) => {
    ...
    try {
      await api.post(ENDPOINTS.ADMIN.SUBSCRIPTION_UPGRADE, { planId: targetPlanId });
    ...
    <ClinicAdminLayout>
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-8">
          <h2 className="text-lg font-bold text-gray-800 mb-2">Current Plan</h2>
          {currentPlan ? (
            <>
              ...
              <p className="text-xs text-gray-500 mt-1">
                ...
                {currentPlan.maxBookingsPerMonth} bookings/month.
              </p>
            </>
          ) : (
              ...
              return (
                <div
                  ...
                  className={`rounded-lg border p-4 text-sm ${
                    isCurrent ? 'border-blue-500 shadow-md' : 'border-gray-200 shadow-sm'
                  }`}
                >
                  ...
                  <ul className="space-y-1 mb-4 text-xs text-gray-600">
                    ...
                    {plan.allowCustomBranding && <li>• Custom branding</li>}
                    {plan.enableReviews && <li>• Patient reviews & ratings</li>}
                    {plan.enableBulkSlots && <li>• Bulk slot creation</li>}
                    {plan.enableExports && <li>• Export bookings (Excel & PDF)</li>}
                    {plan.enableAuditLogs && <li>• Admin audit logs</li>}
                  </ul>
                  ...
                  {isCurrent ? (
                    ...
                    >
                      {upgradingPlanId === plan.id ? 'Upgrading...' : 'Switch to this Plan'}
                    </button>
                  )}
                </div>
              );
